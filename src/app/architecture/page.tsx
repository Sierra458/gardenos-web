...
export const metadata = { title: "Architecture · GardenOS" };

export default async function ArchitectureIndex() {
  const notes = await loadAllNotes(CONTENT_DIR);
  const items = notes.filter(n => n.slug.startsWith("/architecture/"));

  return (
    <div>
      <div className="text-[11px] uppercase tracking-wider text-[var(--color-text-muted)] mb-1">Architecture</div>
      <h1 className="text-[26px] font-semibold tracking-tight mb-6">All docs</h1>
